...
function unavailablePage<T>(feature: string) {
  return (_options: ListPageOptions): Promise<ListPageResult<T>> =>
    migrationPendingAsync<ListPageResult<T>>(feature);
}
...
async function listJsonPage<T extends { id: string }>(
  table: string,
  numberField: string,
  ...
  if (Array.isArray(status)) {
    query = query.in("document->>status", status);
  } else if (status && status !== "all") {
    query = query.eq("document->>status", status);
  }
  if (search) {
    query = query.eq(`document->>${numberField}`, search);
  }

  const { data, error } = await query.range(offset, offset + pageSize);
  if (error) throw error;

  const rows = ((data ?? []) as JsonRow[]).slice(0, pageSize);
  return {
    items: rows.map(rowToDocument<T>),
    nextCursor: (data ?? []).length > pageSize ? offset + pageSize : null,
...
export const supabaseAdapter: DataAdapter = {
  ...
  deliveryOrders: {
    ...
    listPage: (options) => listJsonPage<DeliveryOrder>("delivery_orders", "doNumber", options),
  },
  purchaseOrders: {
    ...
    listPage(options) {
      const status =
        options.status === "pending_receipt"
          ? ["sent", "partial_received"]
          : options.status;
      return listJsonPage<PurchaseOrder>("purchase_orders", "poNumber", options, status);
    },
  ...
  invoices: {
    ...
    async listPage(options) {
      if (options.status === "overdue") {
        const result = await listJsonPage<Invoice>("invoices", "invoiceNumber", {
          ...options,
          status: "sent",
        });
        return {
          ...result,
          items: result.items.filter((invoice) => new Date(invoice.dueDate).getTime() < Date.now()),
        };
      }
      return listJsonPage<Invoice>("invoices", "invoiceNumber", options);
    },
