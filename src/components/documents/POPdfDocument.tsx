...
export function POPdfDocument({ doc, qrDataUrl, logoSrc }: { doc: PurchaseOrder; qrDataUrl?: string; logoSrc?: string }) {
  const out = poOutstanding(doc);
  return (
    <Document title={doc.poNumber} author={COMPANY.name}>
      <Page size="A4" style={s.page}>
        <View style={s.headerRow}>
          ...
          <View style={s.metaRight}>
            ...
            <Text style={s.docNumber}>{doc.poNumber}</Text>
            <Text style={s.smallMuted}>Order date: {formatDate(doc.orderDate)}</Text>
            {doc.expectedDelivery && <Text style={s.smallMuted}>Expected: {formatDate(doc.expectedDelivery)}</Text>}
            <Text style={s.smallMuted}>Status: {PO_STATUS_LABEL[doc.status].toUpperCase()}</Text>
          </View>
        </View>

        <View style={s.sectionRow}>
          <View style={s.card}>
            <Text style={s.cardLabel}>SUPPLIER</Text>
            <Text style={s.cardTitle}>{doc.supplierSnapshot.name}</Text>
            <Text style={s.cardSub}>{doc.supplierSnapshot.address}</Text>
            <Text style={s.cardSub}>{doc.supplierSnapshot.phone}</Text>
          </View>
          <View style={s.card}>
            <Text style={s.cardLabel}>DELIVER TO</Text>
            <Text style={s.cardTitle}>{COMPANY.name}</Text>
            <Text style={s.cardSub}>{COMPANY.address}</Text>
            <Text style={s.cardSub}>{COMPANY.phone}</Text>
          </View>
        </View>
        ...
        <View style={s.table}>
          <View style={s.thead}>
            ...
            <Text style={[s.th, { width: "16%", textAlign: "right" }]}>UNIT COST</Text>
          ...
          {doc.amountPaid > 0 && (
            <>
              <View style={s.totalRow}>
                <Text style={[s.totalLabel, { color: "#047857" }]}>Amount paid</Text>
                <Text style={[s.totalValue, { color: "#047857" }]}>- {currency(doc.amountPaid)}</Text>
              </View>
              <View style={s.grandTotalRow}>
                <Text style={[s.grandLabel, { color: out > 0 ? "#b91c1c" : "#047857" }]}>Balance owed</Text>
                <Text style={[s.grandValue, { color: out > 0 ? "#b91c1c" : "#047857" }]}>{currency(out)}</Text>
              </View>
            </>
          )}
        </View>
        ...
        <View style={s.signRow}>
          ...
          <View style={s.signBlock}><View style={s.signLine} /><Text style={s.signLabel}>APPROVED BY</Text></View>
