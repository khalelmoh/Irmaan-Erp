      ...
      <Page size="A4" style={s.page}>
        <View style={s.headerRow}>
          ...
          <View style={s.metaRight}>
            ...
            <Text style={s.docNumber}>{doc.invoiceNumber}</Text>
            <Text style={s.smallMuted}>Issue date: {formatDate(doc.issueDate)}</Text>
            <Text style={s.smallMuted}>Due date: {formatDate(doc.dueDate)}</Text>
        ...
        <View style={s.sectionRow}>
          ...
          <View style={s.card}>
            <Text style={s.cardLabel}>INVOICE DETAILS</Text>
            <Text style={s.cardSub}>Invoice #: <Text style={{ color: "#0f172a" }}>{doc.invoiceNumber}</Text></Text>
            <Text style={s.cardSub}>Issue: <Text style={{ color: "#0f172a" }}>{formatDate(doc.issueDate)}</Text></Text>
            <Text style={s.cardSub}>Due: <Text style={{ color: "#0f172a" }}>{formatDate(doc.dueDate)}</Text></Text>
            {doc.doIds.length > 0 && (
              <Text style={s.cardSub}>Related D.O: <Text style={{ color: "#0f172a" }}>{doc.doIds.length} order(s)</Text></Text>
            )}
          </View>
        </View>
        ...
        <View style={s.totalsBox}>
          <View style={s.totalRow}>
            ...
            <Text style={s.totalValue}>{currency(doc.subtotal)}</Text>
          </View>
          <View style={s.totalRow}>
            <Text style={s.totalLabel}>Tax ({(doc.taxRate * 100).toFixed(2)}%)</Text>
            <Text style={s.totalValue}>{currency(doc.taxAmount)}</Text>
          </View>
          <View style={s.grandTotalRow}>
            <Text style={s.grandLabel}>Total</Text>
            <Text style={s.grandValue}>{currency(doc.total)}</Text>
          </View>
          {doc.amountPaid > 0 && (
            <>
              <View style={s.totalRow}>
              ...
              <View style={s.grandTotalRow}>
                <Text style={[s.grandLabel, { color: out > 0 ? "#b91c1c" : "#047857" }]}>Balance due</Text>
